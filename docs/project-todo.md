...
2. Fix nabucasa fragility: `hass_nabucasa`'s remote-UI/ACME certificate handler has
   effectively no tolerance for a single transient DNS failure and no automatic retry once
   it gives up, so a brief DNS blip at the wrong moment permanently wedges remote UI setup
   until a manual Core restart happens to land outside the bad window. Full timeline,
   root cause, and open items (including filing upstream against `NabuCasa/hass-nabucasa`)
   in `nabucasa-remote-ui-dns-fragility.md`.
3. Fix Overview C calendar entries
4. Overview A irrigation indicator (not a Homie/HA config bug: Rachio's HA
...
5. ~~Tesla inverter integration~~ Cancelled 2026-08-09: not happening. The two `— °F`
...
12. Build a remotely accessible Homie Dashboard.
