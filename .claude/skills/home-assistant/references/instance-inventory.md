...
- `tablet-home`, titled "Tablet Home": dead. Was the level 1 root for the `Tablet` kiosk user, a four-card grid navigating to the domain dashboards above; superseded within a day by retargeting `Tablet`'s `default_panel` to `vision-sample`. Unreachable from anywhere in the UI now; should be deleted. See `docs/native-dashboards/dashboard-home.md` in the `pdehlke/homeassistant` repo.
