...
- `dashboard-lights` ("Lights"), `dashboard-av` ("A/V"), `dashboard-lennox-home` ("Lennox Home"), `dashboard-alarm-system` ("Alarm System"): the domain dashboards, level 2 area grid plus level 3 leaves. Lights and A/V are generated by `scripts/rebuild-domain-dashboard.py`; Lennox Home and Alarm System are hand-authored with no generator. See [references/lovelace.md](lovelace.md#domain-dashboards-are-generated-not-hand-edited).
- `tablet-home`, titled "Tablet Home": the level 1 root, four domain cards, built for the `Tablet` kiosk user. See `dashboard-tablet-home.md` in the `pdehlke/homeassistant` repo.
- `vision-sample`, **titled "Home"**: the theme's original Vision Sample demo, being built out as the main kiosk dashboard for wall-mounted touch panels instead. `Tablet`'s `default_panel` points here, not at `tablet-home`. Its Lights tab is generated by `scripts/rebuild-home-tab.py`, a Home-specific sibling of `rebuild-domain-dashboard.py`; see [references/lovelace.md](lovelace.md#homes-own-tabs-are-generated-too-by-a-second-script).
