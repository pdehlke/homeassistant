...
Current as of 2026-08-16 (re-check with `lovelace/dashboards/list`, this list grows fast):
...
- `light.primary_suite_lights`, a native HA Light Group helper (not a template light) aggregating the five Primary Suite lights into one entity, created by pde through the UI to give a tile card on the now-retired Home dashboard a single toggle and brightness slider for the whole suite. The entity itself is unaffected by that dashboard's removal.
